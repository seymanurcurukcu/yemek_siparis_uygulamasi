...
const Index = () => {
  ...
  const [extraItems,setExtraItems]=useState(itemExstra)
  ...
  const handleSize = (sizeIndex)=>{
     const different = prices[sizeIndex] - prices[size];
     setSize(sizeIndex);
     changePrice(different);
  }
  ...
  return (
    <div className="flex items-center md:h-[calc(100vh_-_88px)] gap-5 py-20 flex-wrap ">
      ...
      <div className="md:flex-1 md:text-start text-center">
        ...
        {
          extraItems.map((item)=>(
            <label className="flex items-center gap-x-1" key={item.id} onChange={(e) => handleChange(e, item)}>
